...
declare module 'betterncm-api/ncm' {
	export namespace ncm {
		 ...
		 * @returns 当前歌曲的播放信息
		 */
		function getPlayingSong(): PlayingSongInfo | null;
		/**
		 * 获取当前正在播放的歌曲的简要信息
...
declare module 'betterncm-api/index' {
	...
	const BetterNCM: {
		...
		betterncmFetch: (
			relPath: string,
			option?: (RequestInit & { ignoreApiKey?: boolean | undefined }) | undefined,
		) => Promise<Response>;
	};
...
declare interface AudioPlayerLoadInfo {
	activeCode: number;
	code: number;
	duration: number; // 浮点数，单位：秒
	errorCode: number;
	errorString: string;
}

// 上古指令派发器
declare const legacyNativeCmder: {
	/**
	 * 监听歌曲加载
	 */
	appendRegisterCall(
		...
		callback: (trackHash: string, info: AudioPlayerLoadInfo) => void,
	): void;

	/**
	 * 监听播放状态改变 (播放/暂停等)
	 * @param callback 参数 stateString 形如 'trackHash|state|xxx'，中间为播放状态 resume、pause 等
	 */
	appendRegisterCall(
		action: 'PlayState',
		component: 'audioplayer',
		callback: (trackHash: string, stateString: string) => void,
	): void;

	/**
	 * 其他注册
	 */
	appendRegisterCall(action: string, component: string, callback: (...args: any[]) => void): void;

	/**
	 * 移除监听
	 */
	removeRegisterCall: (action: string, component: string, callback: Function) => void;
