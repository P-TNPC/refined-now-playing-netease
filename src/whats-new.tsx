...
export function whatsNew(force = false): void {
	...
	if (!force) {
		...
		if (!changeLog.some(log => compareVersions(log.version, lastVersion) > 0)) return;
	} else {
		lastVersion = '0.0.0';
	}

	const whatsNewContainer = document.createElement('div');
	whatsNewContainer.id = CONTAINER_ID;
	document.body.appendChild(whatsNewContainer);

	const handleClose = () => {
		unmountComponentAtNode(whatsNewContainer);
		whatsNewContainer.remove();
	};
	render(<WhatsNew lastVersion={lastVersion} onClose={handleClose} />, whatsNewContainer);
}
