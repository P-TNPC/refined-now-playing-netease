...
import rawChangeLog from './whats-new.json';

import { compare, compareVersions } from 'compare-versions';
import { useRef } from 'react';
import { render, unmountComponentAtNode } from 'react-dom';

type ChangeAction = 'add' | 'optimize' | 'fix' | 'remove' | 'revert';
type ChangeItem = [ChangeAction, string, boolean];
interface VersionLog {
	version: string;
	changes: ChangeItem[];
}
const changeLog = rawChangeLog as VersionLog[];
function ChangeIcon({ type }: { type: ChangeAction }) {
	switch (type) {
		case 'add':
			return (
...
function WhatsNew(props: { lastVersion: string; onClose: () => void }) {
	...
	return (
		<div className='rnp-whats-new' ref={modelRef}>
			...
			<div className='rnp-whats-new-content'>
				{changeLog
					.filter(({ version }) => compare(version, props.lastVersion, '>'))
					...
					.map(({ version, changes }) => (
						<div className='rnp-whats-new-version'>
							...
							<ul>
								{changes.map(change => (
									<li className={`${change[2] ? 'major' : ''}`}>
										<ChangeIcon type={change[0]} />
										{change[1]}
									</li>
								))}
							</ul>
			...
			<div className='rnp-whats-new-buttons'>
				<button
					className='rnp-whats-new-button'
					onClick={() => {
...
export function whatsNew(force = false): void {
	const STORAGE_KEY = 'refined-now-playing-last-version';
	const CONTAINER_ID = 'refined-now-playing-whats-new';

	if (document.getElementById(CONTAINER_ID)) return;

	const currentVersion = loadedPlugins['RefinedNowPlaying']!.manifest.version;
	let lastVersion = localStorage.getItem(STORAGE_KEY) ?? '0.0.0';

	if (!force) {
		if (compare(lastVersion, currentVersion, '>=')) return;
		localStorage.setItem(STORAGE_KEY, currentVersion);
		if (!changeLog.some(({ version }) => compare(version, lastVersion, '>'))) return;
	} else {
		lastVersion = '0.0.0';
	}
	...
	render(<WhatsNew lastVersion={lastVersion} onClose={handleClose} />, container);
}
